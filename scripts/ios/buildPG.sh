...
if [ "${TRAVIS_REPO_SLUG}/${TRAVIS_BRANCH}" = "openframeworks/projectGenerator/master" ]; then
    ...
    scp -i scripts/id_rsa projectGenerator_ios.zip tests@192.237.185.151:projectGenerator_ios_new.zip
    ssh -i scripts/id_rsa tests@192.237.185.151 "mv projectGenerator_ios_new.zip projectGenerator_ios.zip"
fi
rm scripts/id_rsa
